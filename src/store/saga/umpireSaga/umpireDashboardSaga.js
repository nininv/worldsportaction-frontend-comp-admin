...
export function* umpireDashboardListSaga(action) {
    try {
        ...
        if (result.status === 1) {
            ...
            yield put({
                type: ApiConstants.API_UMPIRE_DASHBOARD_LIST_SUCCESS,
...
export function* umpireSearchSaga(action) {
    try {
        const result = yield call(UserAxiosApi.umpireDashboardList, action.refRoleId, action.competitionID, action.entityTypeId, action.userName);
        if (result.status === 1) {
            // console.log('saga', result)
            yield put({
