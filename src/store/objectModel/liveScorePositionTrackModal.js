...
function getPositionTrackListObject(data, reporting) {
  return {
    ...
    noPlay:
      reporting === 'PERCENT'
        ? ((JSON.parse(data.playDuration) - data.play) / JSON.parse(data.playDuration)) * 100 + '%'
        : JSON.parse(data.playDuration) - JSON.parse(data.bench),
  };
}
...
function getPositionTrackMatchListObject(data, reporting) {
  return {
    ...
    noPlay:
      reporting === 'PERCENT'
        ? ((JSON.parse(data.playDuration) - data.play) / JSON.parse(data.playDuration)) * 100 + '%'
        : JSON.parse(data.bench) / JSON.parse(data.playDuration),
  };
}
...
module.exports = {
  ...
  getPositionTrackMatchListData,
  getPositionTrackMatchListObject,
};
