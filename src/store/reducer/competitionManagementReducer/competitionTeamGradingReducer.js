  ...
  switch (action.type) {
    ...
    case ApiConstants.API_GET_COMPETITION_OWN_PROPOSED_TEAM_GRADING_LIST_SUCCESS:
      ...
      return {
        ...
        compFinalTeamGradingFinalGradesData: isArrayNotEmpty(finalTeamGradingData.finalGrades)
    ...
    case ApiConstants.ONCHANGE_COMPETITION_OWN_PROPOSED_TEAM_GRADING_DATA:
      let finalGradingOnChangeData = JSON.parse(
        JSON.stringify(state.getCompOwnProposedTeamGradingData),
      );
      ...
      if (action.key === 'sortOrder') {
        ...
        finalGradingOnChangeData[action.index][action.key] =
          oldval > action.value ? action.value - 1 : action.value + 1;
    ...
    case ApiConstants.ONCHANGE_COMPETITION_PART_PROPOSED_TEAM_GRADING_DATA:
      let partProposedTeamGradingOnchangeData = JSON.parse(
        JSON.stringify(state.getPartProposedTeamGradingData),
      );
