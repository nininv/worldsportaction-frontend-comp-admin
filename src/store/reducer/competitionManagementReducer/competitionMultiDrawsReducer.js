...
function getDrawsDuplicate(drawsArray, drawsObject) {
  for (const i in drawsArray) {
    if (
      drawsArray[i].drawsId !== drawsObject.drawsId &&
  ...
  if (sameTimeSlotArray.length == 0) {
    ...
    if (process.env.REACT_APP_VENUE_CONFIGURATION_ENABLED === 'true') {
      ...
      if (remainingSlotUnit > 0) {
        let minUnit = 1;
        let unitArray = sameTimeSlotArray
          .map(s => DrawConstant.subCourtHeightUnit[s.subCourt])
          .filter(s => s <= remainingSlotUnit);
        if (unitArray.length > 0) {
          minUnit = Math.min(...unitArray);
        }
        let fieldIds = Object.keys(DrawConstant.subCourtHeightUnit).filter(
          f => DrawConstant.subCourtHeightUnit[f] == minUnit && !existingSubCourts.includes(f),
        );
