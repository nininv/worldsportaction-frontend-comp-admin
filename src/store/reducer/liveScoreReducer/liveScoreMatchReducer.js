  ...
  for (let i in result) {
    ...
    for (let j in userRoleCheck) {
      if (userRoleCheck[j].roleId === 15 || userRoleCheck[j].roleId === 19) {
        for (let k in linkedEntity) {
          let accreditationBadge = getAccreditationValue(
            accreditationArr,
            result[i].accreditationLevelUmpireRefId,
          );
          let obj = {
            name:
              result[i].firstName +
              ...
              linkedEntity[k].name,
            reserveName:
              result[i].firstName + ' ' + result[i].lastName + ' - ' + linkedEntity[k].name,
            id: parseInt(result[i].id + '' + linkedEntity[k].entityId),
            userId: parseInt(result[i].id),
            umpireId: result[i].id,
            entityId: linkedEntity[k].entityId,
...
function liveScoreMatchReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case ApiConstants.API_LIVE_SCORE_ADD_EDIT_MATCH_SUCCESS:
      ...
      state.addEditMatch['extraTimeMainBreak'] =
        data.extraTimeType === 'FOUR_QUARTERS' ? data.extraTimeMainBreak : data.extraTimeBreak;
      state.addEditMatch['extraTimeqtrBreak'] =
        data.extraTimeType === 'FOUR_QUARTERS' ? data.extraTimeBreak : null;
      if (action.result) {
    ...
    case ApiConstants.API_UMPIRE_LIST_SUCCESS:
      ...
      let checkUserData = createUmpireArray(
    ...
    case ApiConstants.API_NEW_UMPIRE_LIST_SUCCESS:
      ...
      let newCheckUserData = createUmpireArray(
        JSON.parse(JSON.stringify(new_user_Data)),
        state.accreditation,
      );
      return {
        ...state,
    ...
    case ApiConstants.API_GET_LIVESCOREMATCH_DETAIL_SUCCESS:
      ...
      let umpireReserveData = isArrayNotEmpty(action.payload.rosters)
        ? checkUmpireRole(action.payload.rosters, 19)
        : null;
      let umpireCoachData = isArrayNotEmpty(action.payload.rosters)
        ? checkUmpireRole(action.payload.rosters, 20)
        : null;
      let umpires_1 = isArrayNotEmpty(action.payload.umpires)
        ? checkUmpireType(action.payload.umpires, 1)
        : null;
      let umpires_2 = isArrayNotEmpty(action.payload.umpires)
        ? checkUmpireType(action.payload.umpires, 2)
        : null;
      ...
      if (umpires_1) {
        ...
        //     }
        // }
        state.umpire1Orag = isArrayNotEmpty(umpires_1.competitionOrganisations)
          ? umpires_1.competitionOrganisations[0].id
          : [];
      ...
      if (umpires_2) {
        ...
        state.umpire2Orag = isArrayNotEmpty(umpires_2.competitionOrganisations)
          ? umpires_2.competitionOrganisations[0].id
          : [];
