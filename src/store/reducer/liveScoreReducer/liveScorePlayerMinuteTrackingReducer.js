...
function liveScorePlayerMinuteTrackingState(state = initialState, action) {
  switch (action.type) {
    ...
    case ApiConstants.API_LIVE_SCORE_UPDATE_PLAYER_MINUTE_RECORD:
      ...
      let findData;
      if (id) {
        findData = trackDataRes.findIndex(
          att => att.playerId === playerId && att.period === period && att.id === id,
        );
      } else {
        findData = trackDataRes.findIndex(
      ...
      if (positionTrack && gameTimeTrack && attndceRecrd != 'MINUTE') {
        let noOfSelectedPosition = getSelectedPosition(
          playerId,
          state.trackResultData,
          state.positionData,
        );
        if (extraKey == 'positionId') {
          if (findData === -1) {
            ...
            positionDuration =
              noOfSelectedPosition.length > 0 ? periodDuration / noOfSelectedPosition.length : 0;

            for (let i in trackDataRes) {
              if (
                trackDataRes[i].playerId == playerId &&
                trackDataRes[i].playedFullPeriod == true
              ) {
                trackDataRes[i]['duration'] = Math.round(positionDuration);
              } else if (
                trackDataRes[i].playerId == playerId &&
                trackDataRes[i].playedFullPeriod == false
              ) {
            ...
            noOfSelectedPosition = getSelectedPosition(
              playerId,
              state.trackResultData,
              state.positionData,
            );
            ...
            for (let i in state.trackResultData) {
              if (
                state.trackResultData[i].playerId == playerId &&
                state.trackResultData[i].playedFullPeriod == true
              ) {
                state.trackResultData[i]['duration'] = Math.round(positionDuration);
              } else if (
                state.trackResultData[i].playerId == playerId &&
                state.trackResultData[i].playedFullPeriod == false
              ) {
          ...
          if (findData === -1) {
            let trackObj = {
              ...
              playerId: playerdata.playerId,
              period: period,
              positionId: playerdata.attendance
                ? playerdata.attendance.positionId
                  ? playerdata.attendance.positionId
                  : 0
                : 0,
            ...
            for (let i in trackDataRes) {
              if (trackDataRes[i].playerId == playerId) {
                trackDataRes[i]['duration'] = selectedData
                  ? Math.round(positionDuration)
                  : Math.round(positionDuration) / 2;
              }
            }
          } else {
            noOfSelectedPosition = getSelectedPosition(playerId, trackDataRes, state.positionData);
            ...
            for (let i in state.trackResultData) {
              if (state.trackResultData[i].playerId == playerId) {
                state.trackResultData[i]['duration'] = selectedData
                  ? Math.round(positionDuration)
                  : Math.round(positionDuration) / 2;
              }
            }
        ...
        if (extraKey == 'positionId') {
          ...
          if (findData === -1) {
            ...
            state.trackResultData[findData]['playedFullPeriod'] =
              selectedData == periodDuration ? true : false;
        ...
        if (extraKey == 'positionId') {
          if (findData === -1) {
            ...
            state.trackResultData[findData]['playedFullPeriod'] =
              positionDuration === periodDuration ? true : false;
            state.trackResultData[findData]['updatedBy'] = parseInt(userId);
            state.trackResultData[findData]['createdBy'] = null;
          }
        }
