  ...
  switch (action.type) {
    ...
    case ApiConstants.API_GET_UMPIRE_PAYMENT_DATA_SUCCESS:
      ...
      return {
        ...
        paymentStatus:
          filterDataForAllSelectedBox.length &&
          result.length &&
          filterDataForAllSelectedBox.length === result.length,
      };
    ...
    case ApiConstants.API_UPDATE_UMPIRE_PAYMENT_DATA:
      ...
      switch (key) {
        case 'allCheckBox':
          const selectedPayments = umpirePaymentArr.map(payment =>
            data
          ...
          const newPaymentTransfer = !!payData.length
            ? payData
                .filter(payment => payment.user && payment.user.stripeAccountId)
                .map(payment => ({
        ...
        case 'clearData':
          const clearedPayments = umpirePaymentArr.map(payment =>
            payment.paymentStatus === 'unpaid' ? { ...payment, selectedValue: false } : payment,
          );
