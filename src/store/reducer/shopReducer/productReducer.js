  ...
  switch (action.type) {
    ...
    case ApiConstants.SHOP_PRODUCT_DETAILS_ONCHANGE:
      if (action.key === 'variantName') {
        ...
        state.productDetailData['tax'] =
          state.productDetailData.taxApplicable === true ? (Number(action.data) * 10) / 100 : 0;
        state.productDetailData[action.key] = action.data;
      } else if (action.key === 'taxApplicable') {
        state.productDetailData[action.key] = action.data;
        state.productDetailData['tax'] =
          action.data === true ? (Number(state.productDetailData.price) * 10) / 100 : 0;
