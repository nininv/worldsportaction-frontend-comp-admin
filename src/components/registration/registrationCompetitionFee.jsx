...
class RegistrationCompetitionFee extends Component {
  ...
  seasonalDataTree = (seasonalPaymentDefaultArray, selectedSeasonalInstalmentDatesArray, selectedSeasonalFeeKey, uponRegKey, isSeasonalUponReg) => {
    const { TreeNode } = Tree;
    return seasonalPaymentDefaultArray.map((seasonalPaymentDefaultArrayItem) => {
      return (
        ...
        >
          {this.SeasonalDataAdvancedNode(seasonalPaymentDefaultArrayItem, selectedSeasonalInstalmentDatesArray, selectedSeasonalFeeKey, uponRegKey, isSeasonalUponReg)}
        </TreeNode>
      );
    });
  };
  ...
  SeasonalDataAdvancedNode(seasonalPaymentDefaultArrayItem, selectedSeasonalInstalmentDatesArray, selectedSeasonalFeeKey, uponRegKey, isSeasonalUponReg) {
    const { TreeNode } = Tree;
  ...
  instalmentUponReg(key, value) {
    ...
    return (
      <TreeNode
        title={this.uponRegCheckBox(value, key)}
        checkable={false}
  ...
  showInstalmentDate(selectedSeasonalInstalmentDatesArray, selectedSeasonalFeeKey) {
    ...
    return selectedSeasonalInstalmentDatesArray.map((selectedSeasonalInstalmentDatesArrayItem, index) => {
      return (
        <TreeNode
          title={this.getInstalmentDate(selectedSeasonalInstalmentDatesArray, selectedSeasonalInstalmentDatesArrayItem, index, selectedSeasonalFeeKey)}
          key={"index"}
          checkable={false}
  ...
  showSeasonalTeamInstalmentDate(selectedSeasonalTeamInstalmentDatesArray) {
    ...
    return selectedSeasonalTeamInstalmentDatesArray.map((selectedSeasonalTeamInstalmentDatesArrayItem) => {
      return (
        <TreeNode
          title={this.getSeasonalTeamDate(selectedSeasonalTeamInstalmentDatesArray, selectedSeasonalTeamInstalmentDatesArrayItem)}
          key={"index"}
          checkable={false}
          className="customize-subtree-middle"
        />
      ...
      <TreeNode
        title={
          <span
            ...
            onClick={(e) => this.props.instalmentDateAction(selectedSeasonalInstalmentDatesArray, "instalmentAddDate")}
  ...
  teamSeasonalRegistrationcode(seasonalPaymentDefaultArrayItem, selectedSeasonalTeamFeeKey, teamSeasonalSchoolRegCode) {
    if (selectedSeasonalTeamFeeKey.includes("8") || selectedSeasonalTeamFeeKey.includes(8)) {
      ...
      return (
        <TreeNode
          title={this.teamSeasonalRegistrationTitle(teamSeasonalSchoolRegCode)} key={"teamSeasonalSchoolRegCode"}
          checkable={false}
  ...
  seasonalTeamaddButton(selectedSeasonalTeamInstalmentDatesArray) {
    ...
    return (
      <TreeNode
        title={
          <span
            style={{ cursor: 'pointer', paddingTop: 0, marginBottom: 20 }}
            onClick={(e) => this.props.instalmentDateAction(selectedSeasonalTeamInstalmentDatesArray, "seasonalTeaminstalmentAddDate")}
            className="input-heading-add-another"
          >
            {AppConstants.addInstalmentDate}
          </span>
  ...
  seasonalTeamDataTree = (seasonalTeamPaymentDefaultArray, selectedSeasonalTeamInstalmentDatesArray, selectedSeasonalTeamFeeKey, uponRegKey, isTeamSeasonalUponReg, teamSeasonalSchoolRegCode) => {
    const { TreeNode } = Tree;
    return seasonalTeamPaymentDefaultArray.map((seasonalTeamPaymentDefaultArrayItem, catIndex) => {
      return (
        ...
        >
          {seasonalTeamPaymentDefaultArrayItem.id != 8 ? this.SeasonalTeamDataAdvancedNode(seasonalTeamPaymentDefaultArrayItem, selectedSeasonalTeamInstalmentDatesArray, selectedSeasonalTeamFeeKey, uponRegKey, isTeamSeasonalUponReg) : null}
  ...
  SeasonalTeamDataAdvancedNode(seasonalTeamPaymentDefaultArrayItem, selectedSeasonalTeamInstalmentDatesArray, selectedSeasonalTeamFeeKey, uponRegKey, isTeamSeasonalUponReg) {
    const { TreeNode } = Tree;
            ...
            <div className="row">
              <div className="col-sm">
                <InputWithHead
                  ...
                  onChange={(e) =>
                    this.onChangePercentageOff(e.target.value, index)
                  }
                  value={item.amount}
                  disabled={this.checkDiscountDisable(item.organisationId)}
                  suffix={
              ...
              <div className="col-sm">
                <div className="comp-buttons-view">
                  <Tooltip
                    ...
                    onMouseEnter={() =>
                      this.setState({
                        tooltipVisibleDraft: isPublished ? true : false,
                      })
                    }
                    onMouseLeave={() =>
  ...
  tabCallBack = (key) => {
    ...
    if (competitionId !== null && competitionId.length > 0) {
      this.setState({
        ...
        divisionState: key == '3' ? true : false,
      });
    }
    this.setDetailsFieldValue();
  };
