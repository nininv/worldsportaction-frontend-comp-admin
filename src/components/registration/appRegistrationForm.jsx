...
class AppRegistrationForm extends Component {
  ...
  onChangeSetParticipantValue = (value, key, index) => {
    ...
    if (key === 'playedBefore' && value == 0) {
      ...
      let memProd = userRegistration.membershipProducts.find(
        x => x.competitionMembershipProductTypeId === value,
      );
      let divisionId = 0;
      let divisionName = '';
      let memDivision = memProd.divisions.find(
        x =>
          x.fromDate != null &&
          x.toDate != null &&
          new Date(userRegistration.dateOfBirth) >= new Date(x.fromDate) &&
          new Date(userRegistration.dateOfBirth) <= new Date(x.toDate),
      );
      ...
      let oldMemProd = userRegistration.membershipProducts.find(
        x =>
          x.competitionMembershipProductTypeId ===
          userRegistration.competitionMembershipProductTypeId,
      );
  ...
  onChangeSetProdMemberTypeValue = (value, index, prodIndex) => {
    ...
    // Enable the existing one and disable the new one
    let oldMemProd = userRegistration.membershipProducts.find(
      x => x.competitionMembershipProductTypeId === product.competitionMembershipProductTypeId,
    );
    if (oldMemProd != null && oldMemProd != '' && oldMemProd != undefined) {
      oldMemProd.isDisabled = false;
    }
  ...
  onChangeSetVolunteerValue = (value, index) => {
    ...
    let filterList = volunteerList.filter(
      x => (x.id === 1 && x.isActive) || (x.id == 3 && x.isActive),
    );
  ...
  onChangeSetVoucherValue = (value, key, index) => {
    ...
    if (key === 'tempParticipantId') {
      let oldTempParticipant = userRegistrations.find(
        x => x.tempParticipantId === oldTempParticipantId,
      );
      if (
        oldTempParticipant != null &&
        oldTempParticipant != undefined &&
        oldTempParticipant != ''
      ) {
  ...
  removeModalPopup = modalOption => {
    if (modalOption === 'ok') {
      if (this.state.modalKey === 'participant') {
        ...
        this.state.modalKey === 'productFriend' ||
        this.state.modalKey === 'productReferFriend'
      ) {
        this.removeFriend();
  ...
  removeParticipant = () => {
    ...
    let deletedTempParticipant = vouchers.find(
      x => x.tempParticipantId === userRegistration.tempParticipantId,
    );

    if (
      deletedTempParticipant != null &&
      deletedTempParticipant != '' &&
      deletedTempParticipant != undefined
    ) {
      let newVouchers = vouchers.filter(
        x => x.tempParticipantId != userRegistration.tempParticipantId,
      );
  ...
  removeFriend = () => {
    ...
    if (this.state.modalKey === 'participantFriend') {
      ...
      userRegistration.products[this.state.productIndex].referFriends.splice(
        this.state.subIndex,
        1,
      );
    }
  ...
  removeVoucher = modalOption => {
    ...
    let tempParticipant = userRegistrations.find(
      x => x.tempParticipantId === voucher.tempParticipantId,
    );
  ...
  headerView = () => (
    <div className="header-view">
      <Header className="form-header-view bg-transparent d-flex align-items-center">
        <Breadcrumb className="align-items-center align-self-center" separator=" > ">
          {/* <NavLink to="/registration">
                        <Breadcrumb.Item className="breadcrumb-product">Products</Breadcrumb.Item>
                    </NavLink> */}
          <Breadcrumb.Item className="breadcrumb-add">{AppConstants.appRegoForm}</Breadcrumb.Item>
        </Breadcrumb>
      </Header>
    </div>
  );
      ...
      {this.state.registeringYourself == 3 && (
        <div>
          ...
          <Radio.Group
            className="reg-competition-radio"
            onChange={e =>
              this.onChangeSetParticipantValue(e.target.value, 'whoAreYouRegistering', index)
            }
          ...
          <Radio.Group
            className="reg-competition-radio"
            onChange={e =>
              this.onChangeSetParticipantValue(e.target.value, 'whatTypeOfRegistration', index)
            }
    ...
    return (
      <div className="formView content-view pt-5">
        ...
        <div className="applicable-to-text d-flex">
          ...
          {index == 0 && <div className="another-competition">Find Another Competition</div>}
        </div>
        ...
        >
          <Select
            ...
            onChange={e =>
              this.onChangeSetParticipantValue(e, 'competitionMembershipProductTypeId', index)
            }
      ...
      <div className="formView content-view pt-5">
        ...
        >
          <InputWithHead
            ...
            onChange={e =>
              this.onChangeSetParticipantValue(e.target.value, 'emergencyContactName', index)
            }
        ...
        >
          <InputWithHead
            ...
            onChange={e =>
              this.onChangeSetParticipantValue(e.target.value, 'emergencyContactNumber', index)
            }
  ...
  parentGuardianView = (item, index) => {
    ...
    return (
      <div className="formView content-view pt-5">
        <span className="form-heading">{AppConstants.parents_guardians}</span>
        <Form.Item
  ...
  additionalPersonalInfoView = (item, index) => {
    ...
    return (
      <div className="formView content-view pt-5">
        <span className="form-heading">{AppConstants.additionalPersonalInfoReqd}</span>
        {regSetting.played_before === 1 && (
          <div>
            <span className="applicable-to-heading">{AppConstants.haveYouEverPlayed}</span>
            <Radio.Group
              className="reg-competition-radio"
              onChange={e =>
                this.onChangeSetParticipantValue(e.target.value, 'playedBefore', index)
              }
              ...
              {item.playedBefore && (
                <div className="pl-5 pb-5">
                  <InputWithHead
                    ...
                    onChange={e =>
                      this.onChangeSetParticipantValue(e.target.value, 'playedYear', index)
                    }
                  ...
                  <InputWithHead
                    ...
                    onChange={e =>
                      this.onChangeSetParticipantValue(e.target.value, 'playedClub', index)
                    }
                  ...
                  <InputWithHead
                    ...
                    onChange={e =>
                      this.onChangeSetParticipantValue(e.target.value, 'playedGrade', index)
                    }
        ...
        {regSetting.last_captain === 1 && (
          <div>
            <span className="applicable-to-heading">{AppConstants.lastCaptainName}</span>
            <InputWithHead
              heading={AppConstants.fullName}
              placeholder={AppConstants.lastCaptainName}
              onChange={e =>
                this.onChangeSetParticipantValue(e.target.value, 'lastCaptainName', index)
              }
      ...
      <div className="formView content-view pt-5">
        ...
        >
          {(playerPositionList || []).map(play1 => (
            <Option key={'playerPosition1_' + play1.id} value={play1.id}>
              {play1.name}
            </Option>
          ))}
        </Select>
        ...
        >
          {(playerPositionList || []).map(play2 => (
            <Option key={'playerPosition2_' + play2.id} value={play2.id}>
              {play2.name}
            </Option>
          ))}
        </Select>
  ...
  playWithFriendView = (item, index, participantOrProduct, productIndex) => {
    ...
    return (
      <div className="formView content-view pt-5">
        ...
        <span className="form-heading" style={{ fontSize: '10px' }}>
          {AppConstants.playWithFriendSubtitle}
        </span>
          ...
          <div key={'friend' + friendIndex} className="inside-container-view pt-0">
            <div className="row">
              ...
              <div
                ...
                onClick={() =>
                  this.deleteEnableOrDisablePopup(
                    participantOrProduct,
                    true,
                    index,
                    productIndex,
                    friendIndex,
                    AppConstants.friendDeleteConfirmMsg,
                    'Friend',
                  )
                }
              >
      ...
      <div className="formView content-view pt-5">
        ...
        >
          ...
          >
            {(heardByList || []).map(heard => (
              <Radio key={'heardBy_' + heard.id} value={heard.id}>
                {heard.description}
              </Radio>
            ))}
          </Radio.Group>
        ...
        {item.heardByRefId == 5 && (
          <div className="pl-5 pr-5">
            <InputWithHead
              ...
              onChange={e =>
                this.onChangeSetParticipantValue(e.target.value, 'heardByOther', index)
              }
        ...
        >
          ...
          >
            {(favouriteTeamsList || []).map(fav => (
              <Option key={'favouriteTeam_' + fav.id} value={fav.id}>
                {fav.description}
              </Option>
            ))}
          </Select>
            ...
            >
              ...
              >
                {(firebirdPlayerList || []).map(fire => (
                  <Option key={'firebirdPlayer_' + fire.id} value={fire.id}>
                    {fire.description}
                  </Option>
                ))}
              </Select>
        ...
        {regSetting.photo_consent === 1 && (
          <Checkbox
            ...
            onChange={e =>
              this.onChangeSetParticipantValue(e.target.checked, 'isConsentPhotosGiven', index)
            }
        ...
        {regSetting.disability === 1 && (
          <div>
            ...
            <Radio.Group
              className="reg-competition-radio"
              onChange={e =>
                this.onChangeSetParticipantValue(e.target.value, 'isDisability', index)
              }
  ...
  otherInfoReqdView = () => {
    ...
    return (
      <div className="formView content-view pt-5">
        <span className="form-heading">{AppConstants.OtherInfoReqd}</span>
        {regSetting.club_volunteer === 1 && (
          <div>
            <span className="applicable-to-heading">{AppConstants.yourSupportImportant} </span>
            <div className="fluid-width">
              <div className="row">
                <div className="col-sm">
  ...
  uniformAndMerchandise = () => {
    return (
      <div className="formView content-view pt-5">
        <span className="form-heading pb-4">{AppConstants.uniformAndMerchandise}</span>
      </div>
    );
  };
                  ...
                  <div
                    ...
                    onClick={() =>
                      this.deleteEnableOrDisablePopup(
                        ...
                        0,
                        index,
                        AppConstants.voucherDeleteConfirmMsg,
                      )
                    }
                  >
                    <span className="user-remove-btn">
  ...
  contentView = () => {
    ...
    return (
      <div>
        <div className="mb-20">{this.registeringYourselfView()}</div>

        {(userRegistrations || []).map((item, index) => (
          <div key={'userReg' + index}>
            {this.dividerTextView('PARTICIPANT ' + (index + 1), styles, 'participant', index, -1)}
            <div className="mb-20">{this.registrationQuestionView(item, index)}</div>
            {item.isPlayer != -1 && (
              <div>
                <div className="mb-20">{this.membershipProductView(item, index)}</div>
                <div className="mb-20">{this.participantDetailView(item, index)}</div>
                {getAge(item.dateOfBirth) <= 18 && (
                  <div className="mb-20">{this.parentGuardianView(item, index)}</div>
                )}
                {(regSetting.last_captain === 1 || regSetting.played_before === 1) && (
                  <div className="mb-20">{this.additionalPersonalInfoView(item, index)}</div>
                )}
                {item.isPlayer === 1 && (
                  <div>
                ...
                {(item.products || []).map((prod, prodIndex) => (
                  <div key={'prod' + prodIndex}>
                    {this.dividerTextView(
                      'PARTICIPANT ' + (index + 1) + ' - MEMBERSHIP ' + (prodIndex + 1),
                      stylesProd,
                      'product',
                      index,
                      prodIndex,
                    )}
                    <div>{this.membershipProductProductView(item, prod, prodIndex, index)}</div>
                    {prod.isPlayer && (
                      <div>
                        {regSetting.nominate_positions === 1 && (
                          <div className="mb-20">
                ...
                <div className="mb-20">{this.additionalInfoView(item, index)}</div>
              </div>
            )}
          </div>
        ))}
        {this.state.registeringYourself != 0 &&
          ...
          userRegistrations[0].isPlayer != -1 && (
            <div>
              ...
              {(regSetting.language === 1 ||
                ...
                regSetting.country === 1 ||
                regSetting.club_volunteer === 1) && (
                <div className="mb-20">{this.otherInfoReqdView()}</div>
              )}
              {regSetting.shop === 1 && <div className="mb-20">{this.uniformAndMerchandise()}</div>}
              {regSetting.voucher === 1 && <div>{this.voucherView()}</div>}
              <Form.Item
              ...
              >
                <div className="formView" style={{ background: 'none' }}>
                  ...
                  >
                    ...
                    <span className="app-reg-terms">{AppConstants.termsAndConditions} </span>
                  </Checkbox>
                </div>
              </Form.Item>
            </div>
