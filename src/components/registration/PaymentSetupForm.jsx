  ...
  const handleSubmit = async event => {
    ...
    try {
      ...
      const result = await userHttp.get(
        `${process.env.REACT_APP_USER_API_URL}/becs/secret/${orgDetail.organisationId}`,
      );
      secret = result.data;
    } catch (e) {
      console.log({ e });
    ...
    const result = await stripe.confirmAuBecsDebitSetup(secret, {
      payment_method: {
    ...
    if (result.error) {
      ...
      await userHttp.get(
        `${process.env.REACT_APP_USER_API_URL}/becs/confirm/${orgData.organisationId}`,
      );
      // const orgData = await getOrganisationData()
