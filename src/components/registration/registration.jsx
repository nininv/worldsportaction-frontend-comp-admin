...
const columns = [
  ...
  },
  {
    ...
    render: (isUsed, record, index) =>
      (isArrayNotEmpty(record.actionViews) &&
...
class Registration extends Component {
  ...
  componentDidUpdate(nextProps) {
    ...
    if (
      nextProps.liveScoreDashboardState != this.props.liveScoreDashboardState ||
      nextProps.registrationDashboardState != this.props.registrationDashboardState
    ) {
      if (
  ...
  handleRegTableList = async page => {
    ...
    const yearRefId =
      getGlobalYear() && this.state.yearRefId != -1
  ...
  setSchoolInvoiceFailed = record => {
    this.setState({
      ...
      actionView: 4,
      modalMessage: AppConstants.regFailedModalMsg,
