...
class AddTeamMember extends Component {
    ...
    getDivisions = () => {
        try {
            ...
            const membershipProduct = membershipProducts.find(x => x.competitionMembershipProductId == team.competitionMembershipProductId && x.competitionMembershipProductTypeId == team.competitionMembershipProductTypeId);
            const division = membershipProduct.divisions.find(x => x.competitionMembershipProductDivisionId == team.competitionMembershipProductDivisionId);
            if (division) {
                return [{
    ...
    checkGenderDivisionRestriction = (teamMembersSaveTemp) => {
        try {
            ...
            for (let org of membershipProductInfo) {
                const competition = org.competitions.find(x => x.competitionUniqueKey == teamMembersSaveTemp.competitionId);
                if (competition) {
                    competitionName = competition.competitionName;
                }
            }
            const selectedDivision = teamMembersSaveTemp.divisions[0];
            const gender = selectedDivision.genderRefId == 1 ? "Female" : selectedDivision.genderRefId == 2 ? "Male" : "Non-binary";
            let personNames = [];
            let errorMessage = '';
    ...
    onSaveClick = () => {
        try {
            ...
            const isGenderDivisionRestrictionError = this.checkGenderDivisionRestriction(teamMembersSaveTemp);
    ...
    teamMemberView = (teamMember, teamMemberIndex) => {
        try {
            ...
            const { teamMembersSave, personalData } = this.props.userState;
            const teamMembers = teamMembersSave?.teamMembers ? teamMembersSave?.teamMembers : [];
            const dateOfBirth = personalData.dateOfBirth ? moment(personalData.dateOfBirth, "YYYY-MM-DD").format("MM-DD-YYYY") : null;
            return (
                <div className="light-grey-border-box mt-0">
                    ...
                    {(teamMember.membershipProductTypes || []).map((product) => (
                        <Checkbox
                            ...
                            onChange={(e) => {
                                const prodIndexTemp = teamMember.membershipProductTypes.findIndex((x) => x.competitionMembershipProductTypeId === product.competitionMembershipProductTypeId && x.competitionMembershipProductId === product.competitionMembershipProductId);
