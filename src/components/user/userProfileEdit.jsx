...
class UserProfileEdit extends Component {
  ...
  async componentDidMount() {
    if (this.props.history.location.state) {
      ...
      const showSameEmailOption =
        !!data.dateOfBirth &&
        moment().year() - moment(data.dateOfBirth).year() < 18 &&
        (titleLabel === AppConstants.edit + ' ' + AppConstants.address ||
          titleLabel === AppConstants.edit + ' ' + AppConstants.child ||
          titleLabel === AppConstants.addChild);
      ...
      if (showSameEmailOption) {
        const matchingData = parentData.find(parent => {
          return (
            parent.email?.toLowerCase() + '.' + data.firstName?.toLowerCase() ===
      ...
      if (
        personalEmail?.toLowerCase() + '.' + data.firstName?.toLowerCase() ===
        data.email?.toLowerCase()
      ) {
      ...
      if (userData.isInActive) {
        ...
        if (this.state.titleLabel === AppConstants.edit + ' ' + AppConstants.address) {
          ...
          await this.setUserDataContactEmail(
            parentData[0].email + '.' + this.state.userData.firstName,
          );
        } else {
      ...
      <div className="content-view pt-0">
        <div className="row">
          <div className="col-sm">
            ...
            <div>
              <InputWithHead
                heading={AppConstants.nationalAccreditationLevelUmpireQ}
                required="required-field"
              />
              <Form.Item
            ...
            <div>
              <InputWithHead
                heading={AppConstants.nationalAccreditationLevelCoachQ}
                required="required-field"
              />
              <Form.Item
  ...
  addChildOrParent = selectedMatch => async () => {
    ...
    const sameEmail =
      isSameEmail || userData.email === this.props.history.location.state.userData.email ? 1 : 0;
    ...
    return (
      <div className="comp-dash-table-view mt-5">
        ...
        <div className="d-flex align-items-center justify-content-between mt-4">
          ...
          <Button type="primary" onClick={this.addChildOrParent(selectedMatch)}>
            {AppConstants.next}
          </Button>
        </div>
      </div>
