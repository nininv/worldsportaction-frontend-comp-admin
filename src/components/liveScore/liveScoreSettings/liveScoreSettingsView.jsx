...
class LiveScoreSettingsView extends Component {
    ...
    componentDidUpdate(nextProps) {
        ...
        if (nextProps.appState !== appState) {
            if (appState.onLoad === false && yearLoading === true) {
                const yearId = appState.yearList.length > 0
                    ? getCurrentYear(appState.yearList)
                    : null;
    ...
    handleSubmit = () => {
        ...
        if (invitedAnyAssoc.length > 0) {
            if (
                JSON.stringify(invitedAnyAssocArr)
                === JSON.stringify(invitedAnyAssoc)
            ) {
                assocValue = false;
            } else {
                assocValue = true;
        ...
        if (invitedAnyClub.length > 0) {
            if (
                JSON.stringify(invitedAnyClubArr)
                === JSON.stringify(invitedAnyClub)
            ) {
                clubValue = false;
            } else {
                clubValue = true;
        ...
        const attendenceRecordingTime = this.getRecordingTime(
            days,
            hours,
            minutes,
        );
    ...
    differentPositionTracking = (options, selectedOption) => {
        if (
            options.value === "gameTimeTracking"
            && selectedOption.includes("gameTimeTracking")
        ) {
            return (
                <div className="pt-4">
                    <Select
                    ...
                    >
                        {trackFullPeriod.map((item) => (
                            <Option
                                key={`trackFullPeriod_${item.value}`}
                                value={item.value}
                            >
                                {item.name}
                            </Option>
            ...
            <div>
                <div className="formView content-view pt-4 mb-5">
                    ...
                    >
                        <InputWithHead
                            ...
                            onBlur={(e) => {
                                this.formRef.current.setFieldsValue({
                                    competition_name: captializedString(
                                        e.target.value,
                                    ),
                                });
                            }}
                            ...
                            <>
                                ...
                                >
                                    {this.props.liveScoreSetting.venueData
                                        && this.props.liveScoreSetting.venueData.map(
                                            (item) => (
                                                <Option
                                                    key={`venue_${item.venueId}`}
                                                    value={item.venueId}
                                                >
                                                    {item.venueName}
                                                </Option>
                        ...
                        <div className="row">
                            <div className="col-sm-12">
                                <Checkbox.Group
                                    ...
                                    value={record1}
                                    onChange={(val) => this.handleCheckBoxChange(
                                        "record1",
                                        val,
                                    )}
                                >
                                ...
                                rules={[
                                    {
                                        required: true,
                                        message:
                                            ValidationConstants.recordUmpireField,
                                    },
                                ]}
                            >
                                <Select
                ...
                <div className="formView content-view pt-4 mb-5">
                    ...
                    <span className="text-heading-large pt-5">
                        {AppConstants.attendance_record_report}
                    </span>
                                ...
                                rules={[
                                    {
                                        required: true,
                                        message:
                                            ValidationConstants.attendanceRecordField,
                                    },
                                ]}
                            >
                                <Select
                                ...
                                rules={[
                                    {
                                        required: true,
                                        message:
                                            ValidationConstants.attendanceReportField,
                                    },
                                ]}
                            >
                                <Select
                    ...
                    <InputWithHead
                        isOptional
                        heading={AppConstants.attendanceRecordingTime}
                    />
                    ...
                    <InputWithHead
                        isOptional
                        heading={AppConstants.playerBorrowing}
                    />
                    <div className="row mt-0 ml-1">
                        <Radio.Group
                            ...
                            <div className="row mt-0">
                                <div className="col-sm-12">
                                    ...
                                    {borrowedPlayer === "GAMES" && (
                                        <div
                                            className="inside-container-view w-100"
                                            style={{ marginTop: 15 }}
                                        >
                    ...
                    <div className="row">
                        <div className="col-sm-12">
                            ...
                            <Checkbox
                                ...
                                onChange={(e) => this.onChangeLineUpSelection(
                                    e.target.checked,
                                    record1,
                                )}
                            ...
                            >
                                <span className="checkbox-text">
                                    ...
                                    <i className="input-heading__optional">
                                        - optional
                                    </i>
                                </span>
                            </Checkbox>
                            ...
                            {lineupSelection && (
                                <div
                                    className="inside-container-view pt-0"
                                    style={{ marginTop: 15 }}
                                >
                                    <div className="row">
                                        <div className="col-sm">
    ...
    associationSearchInvitee = () => {
        ...
        const disabledComponent = (this.state.isEdit === "edit" || this.state.edit === "edit")
    ...
    render() {
        ...
        const localId = isUmpireDashboardScreen
            ? null
            : getLiveScoreCompetiton();
