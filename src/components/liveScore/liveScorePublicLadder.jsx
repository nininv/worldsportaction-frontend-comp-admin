...
class LiveScorePublicLadder extends Component {
  ...
  componentDidUpdate(nextProps) {
    if (nextProps.liveScoreFixtureCompState !== this.props.liveScoreFixtureCompState) {
      if (this.state.onCompLoad && this.props.liveScoreFixtureCompState.onLoad == false) {
        ...
        let compKey =
          this.props.liveScoreFixtureCompState.comptitionList &&
          this.props.liveScoreFixtureCompState.comptitionList[0].competitionUniqueKey;
    ...
    if (this.props.liveScoreLadderState !== nextProps.liveScoreLadderState) {
      if (this.props.liveScoreLadderState.onLoad == false && this.state.onDivisionLoad) {
        if (this.props.liveScoreLadderState.liveScoreLadderDivisionData.length > 0) {
          ...
          this.props.liveScoreLaddersListAction(
            this.state.selectedComp,
            division,
            this.state.competitionUniqueKey,
          );
        }
      }
    }
  ...
  dropdownView = () => {
    ...
    let competition = this.props.liveScoreFixtureCompState.comptitionList
      ? this.props.liveScoreFixtureCompState.comptitionList
      : [];
    let division = isArrayNotEmpty(liveScoreLadderState.liveScoreLadderDivisionData)
  ...
  contentView = () => {
    const {
      liveScoreLadderState: { liveScoreLadderListData, onLoad },
    } = this.props;
    return (
