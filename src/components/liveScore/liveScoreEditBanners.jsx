...
import { Breadcrumb, Button, Checkbox, Form, Layout, message, Radio, Spin } from "antd";
...
class LiveScoreEditBanners extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (!this.state.isEdit) {
      if (this.backUrl === 'communication' && this.state.stateBannerCnt >= 0 && bannerResult.length >= this.state.stateBannerCnt) {
        this.goBack();
        message.warning("You are going to have more state banners than how many you can have.");
      } else if (this.backUrl !== 'communication' && this.state.compBannerCnt >= 0 && bannerResult.length >= this.state.compBannerCnt) {
        this.goBack();
        message.warning("You are going to have more competition banners than how many you can have.");
      }
    }
    ...
    if (!this.props.userState.onLoad && this.props.userState.status) {
      ...
      if (prevBannerCount) {
        if (bannerCount && (prevBannerCount.stateBannerCnt !== bannerCount.stateBannerCnt
          || prevBannerCount.compBannerCnt !== bannerCount.compBannerCnt
          || this.state.compBannerCnt + this.state.compBannerCnt < 0)) {
  ...
  onUploadButton = () => {
    ...
    if (id !== null || this.backUrl === 'communication') {
      ...
      this.props.liveScoreAddBanner(organisationId, this.backUrl === 'communication' ? null : id, this.state.bannerImgSend, showOnhome, showOndraws, showOnladder, showOnnews, showOnchat, format, bannerLink, bannerId);
    }
...
function mapDispatchToProps(dispatch) {
  return bindActionCreators({
    liveScoreAddBanner, liveScoreAddBannerUpdate, clearEditBannerAction, getBannerCnt, getliveScoreBanners,
  }, dispatch);
}
