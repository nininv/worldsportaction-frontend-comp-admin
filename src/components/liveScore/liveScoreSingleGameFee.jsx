...
import { Layout, Breadcrumb, Button, Modal, Table, Menu, Pagination } from 'antd';
  ...
  },
  {
    ...
    render: (status, record) => (
      <span className="input-heading-add-another pt-0">
        {Number(record.matchesCount) - Number(record.redeemCount)}
      </span>
    ),
  },
  {
    ...
    render: (data, record) => (
      ...
      >
        ...
        >
          {Number(record.matchesCount ? record.matchesCount : 0) >
            Number(record.redeemCount ? record.redeemCount : 0) && (
  ...
  redeemModalView() {
    ...
    return (
      ...
      >
        ...
        <div>
          {' '}
          {AppConstants.gamesRedeemed} : {redeemCount}
        </div>
        <InputWithHead
