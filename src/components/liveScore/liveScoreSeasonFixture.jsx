      ...
      if (record.isRoundChnage) {
        return (
          <div className="table-live-score-table-fixture-style-main">
            <div
              className="table-live-score-table-fixture-style"
        ...
        return <span>{startTime ? liveScore_MatchFormate(startTime) : ''}</span>;
      }
    },
  },
  {
  ...
  },
  {
    ...
    render: (venueCourt, record) => {
      if (record.isRoundChnage) {
        ...
        return <span>{venueCourt ? venueCourt.name : ''}</span>;
      }
    },
  },
  {
  ...
  },
  {
    ...
    render: (matchStatus, record) => {
      if (record.isRoundChnage) {
        ...
        return <span>{matchStatus ? matchStatus : ''}</span>;
      }
    },
  },
];
...
class LiveScoreSeasonFixture extends Component {
  ...
  componentDidUpdate(nextProps) {
    if (nextProps.liveScoreFixtureCompState !== this.props.liveScoreFixtureCompState) {
      if (this.state.onCompLoad && this.props.liveScoreFixtureCompState.onLoad === false) {
        let firstComp =
          this.props.liveScoreFixtureCompState.comptitionList &&
          this.props.liveScoreFixtureCompState.comptitionList[0].id;
  ...
  dropdownView = () => {
    let competition = this.props.liveScoreFixtureCompState.comptitionList
      ? this.props.liveScoreFixtureCompState.comptitionList
      : [];
    let division = this.props.liveScoreMatchState.divisionList
      ? this.props.liveScoreMatchState.divisionList
      : [];
    return (
  ...
  render() {
    return (
      <div className="fluid-width default-bg">
        ...
        <Loader
          visible={
            this.props.liveScoreFixtureCompState.onLoad || this.props.liveScoreMatchState.onLoad
          }
        />
        <Layout>
          {this.headerView()}
