...
import { Layout, Breadcrumb, Select, Button, Form, Checkbox, Radio, Modal } from 'antd';
...
class UmpirePaymentSetting extends Component {
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    if (!isEqual(prevProps.umpireCompetitionState, this.props.umpireCompetitionState)) {
      ...
      if (!this.props.umpireCompetitionState.onLoad) {
        const competitionList = isArrayNotEmpty(
    ...
    if (
      !isEqual(
        this.props.umpirePoolAllocationState.umpirePoolData,
        prevProps.umpirePoolAllocationState.umpirePoolData,
      )
    ) {
      const reqData = {
        organisationId,
    ...
    if (
      !isEqual(this.props.umpirePaymentSettingState, prevProps.umpirePaymentSettingState) &&
  ...
  modifyGetSettingsData = () => {
    ...
    const umpirePaymentSettingsArray =
      !!umpirePaymentSettings && !!umpirePaymentSettings.length
    ...
    const affiliateViewSettingsArray =
      !isOrganiserView && !umpirePaymentSettings.length
  ...
  handleChangeRateCell = (
    ...
    const rateLineDataForChange = rateList.find(data => data[cellLineIdKey] === rateLineDataId)
      ?.rates;
  ...
  handleSave = () => {
    ...
    const allowedDivisionsSetting =
      (!!affiliateSettingArray[0]?.divisions.length || !!affiliateSettingArray[0]?.allDivisions) &&
      allowPayment
            ...
            {!!allowedDivisionList &&
              ...
              hasSettings && (
                <div className="row mb-5 position-absolute">
                  <div className="col-sm" onClick={() => this.handleAddBox()}>
